defmodule FritzAhaApi.Auth do
  ...
  def login(password) do
    :inets.start
    session = with {:ok, {{_type, _statuscode, _reason}, _headers, body}} <- :httpc.request('http://fritz.box/login_sid.lua'),
  ...
  def calculate_response(password,challenge) do
    encoded = :unicode.characters_to_binary(challenge ++ '-' ++ password,:utf8, {:utf16,:little})
