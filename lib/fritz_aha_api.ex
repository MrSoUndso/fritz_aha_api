defmodule FritzAhaApi do
  def call_api(cmd, session, device \\ nil) do
    url =
      case device do
        nil ->
          "http://fritz.box/webservices/homeautoswitch.lua?switchcmd=#{cmd}&sid=#{session.sid}"

        _ ->
          "http://fritz.box/webservices/homeautoswitch.lua?ain=#{device.identifier}&switchcmd=#{
            cmd
          }&sid=#{session.sid}"
      end
    ...
    case :httpc.request(url) do
      {:ok, {{_type, 200, _reason}, _headers, body}} ->
        {:ok, body}

      {:ok, {{_type, statuscode, reason}, _headers, _body}} ->
